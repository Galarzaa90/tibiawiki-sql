...
def fetch_article_images(con, article_list, table, no_title=False, extension=".gif") -> Tuple[int, int, int, int]:
    ...
    with click.progressbar(length=len(article_list), label="Fetching images", show_pos=True) as bar:
        while True:
...
def fetch_articles(article_list):
    i = 0
    with click.progressbar(length=len(article_list), label="Fetching articles") as bar:
        while True:
            if i > len(article_list):
                break
            params = {
